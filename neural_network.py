...
class convulation_block(nn.Module):
    def __init__ (self):
        ...
        self.convulation1=nn.Conv2d(18,256,3,stride=1,padding=1)#1 square status,256 channel
        self.batchnorm1=nn.BatchNorm2d(256)
        
    def forward(self,state):
        state=state.view(-1,18,8,8) #8x8 board,14 square status
...
class output_block(nn.Module):
    ...
    def _calculate_value(self):  
        ...
        self.connection1=nn.Linear(8*8,256)#8x8 board 256 options
        self.bathchnormcont=nn.BatchNorm1d(256)
        self.connection2=nn.Linear(256,1)#What is the value of best move
    
    def _find_policy(self):
        self.convulation_policy=nn.Conv2d(256,128,kernel_size=1)
        self.batchnorm_policy = nn.BatchNorm2d(128)
        self.connection_policy=nn.Linear(128*8*8,4672)
    
    def forward(self,state):
        value_of_state=nnf.relu(self.batchnorm_result(self.convulation_result(state)))
        value_of_state=value_of_state.view(-1,1*8*8)
        ...
        policy_of_state= policy_of_state.view(-1, 128 * 8 * 8)
...
class chess_neural_network(nn.Module):
    def __init__(self) :   
        ...
        self.residualblocks=nn.ModuleList([residual_block() for i in range(5)])
