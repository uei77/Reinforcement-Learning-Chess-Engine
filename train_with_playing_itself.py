...
def play_move(board, model, device, simulation=100, is_Train=True, move_count=0):
    try:
...
def run_self_play(model, device, simulation_move_limit=100):
    board = chess.Board()
    game_history = []
    while not board.is_game_over():
       states = board.copy()      
       move, policy = play_move(board, model, device, simulation=simulation_move_limit)  
       game_history.append((states, policy))
       board.push(move) 
    result = get_game_result(board)      
    return prepare_data(game_history, result)

def manager(model, optimizer, device, iterations=10, games_per_iteration=5, simulation=100):
    model.to(device)
    ...
    for iter in range(iterations):
        ...
        for game in range(games_per_iteration):
            states, policies, values = process_game(model, device, game, simulation=simulation)
        ...
        if len(replay_buffer_states) > 0:
            ...
            win_rate = evaluate_models(model, opponent_model, device, games=4, simulation=20)
...
def model_vs_model(model_white, model_black, device, simulation=20):
    board = chess.Board()
    while not board.is_game_over():
        move_count = len(board.move_stack)
        if board.turn == chess.WHITE:
            move = play_move(board, model_white, device, simulation=simulation, is_Train=False, move_count=move_count)[0] 
        else:
            move = play_move(board, model_black, device, simulation=simulation, is_Train=False, move_count=move_count)[0]
        if move is None:
            break
        board.push(move)
    return get_game_result(board)

def evaluate_models(candidate_model, opponent_model, device, games=10, simulation=20):
    candidate_wins = 0
    opponent_wins = 0
    draws = 0
    for i in range(games):
        if i % 2 == 0:
            result = model_vs_model(candidate_model, opponent_model, device, simulation)
            if result == 1.0: candidate_wins += 1
            elif result == -1.0: opponent_wins += 1
            else: draws += 1            
        else:
            result = model_vs_model(opponent_model, candidate_model, device, simulation)
            if result == -1.0: candidate_wins += 1
            elif result == 1.0: opponent_wins += 1
            else: draws += 1
...
def process_game(model, device, game_idx, simulation=100):
    model.eval() 
    board = chess.Board()
    game_history = []
    with torch.no_grad():
        while not board.is_game_over():
            ...
            with autocast(device_type=device.type, dtype=torch.float16):
                move, policy = play_move(board, model, device, simulation=simulation, move_count=current_move_count)  
